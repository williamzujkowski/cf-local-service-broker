...
| POSTGRES_BROKER_SHARED_OWNER_ROLE | No | true                            | Shared-owner-role model (issue #10). When true (the default), Provision creates a per-instance NOLOGIN group role and makes it the DB owner; Bind grants membership and `ALTER ROLE … SET ROLE` so every CREATE TABLE lands under the shared role. The net effect: multi-binding apps (API + worker) get cross-binding object access without admin-level reconciles. Set to `false` for the legacy per-binding-only behavior (in-place upgrades that don't want to rebind every instance). Unbind and Deprovision auto-detect whether each instance has a shared owner role (issue #12) so existing v0.1.x-provisioned instances still tear down cleanly after a broker upgrade |
